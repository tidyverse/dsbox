---
title: "Tutorial 01 - Airbnb Listings in Edinburgh"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
# make listings data frame globally available
listings <- read_csv("https://rb.gy/guyp0s")
...
The data is in a file called `listings.csv` and it's available at https://rb.gy/guyp0s. The `.csv` suffix (the file extension) indicates that this is a **c**omma **s**eparated **v**alues file. Each line of a CSV file is a data record (an observation in the sample), and each column is a variable. The columns are separated by commas, hence the name. CSV is a commonly used format for storing rectangular datasets.

You can load this file into R using the `read_csv()` function and assign it to an object called `listings`.

```{r load-data, exercise=TRUE}
listings <- read_csv("https://rb.gy/guyp0s")
```

Then, view the data using
...
```{r quiz-rows}
question("What does each row in the `listings` dataset represent?",
...
```{r listings-nrow, exercise=TRUE}

```

```{r listings-nrow-hint-1}
nrow(___)
```

```{r listings-nrow-check}
grade_result(
...
```{r listings-names, exercise=TRUE}
names(listings)
```
...
ggplot(data = listings, mapping = aes(x = ___)) +
  geom_histogram() +
  labs(
    x = "Airbnb listing price, in £",
    y = "Frequency",
...
quiz(
  caption = "",
  question("Which of the following describes the shape of the distribution of nightly rates of Airbnb listings in Edinburgh? Check all that apply.",
           answer("Right skewed",
                  correct = TRUE),
           answer("Left skewed",
...
ggplot(data = listings, mapping = aes(x = review_scores_rating)) +
  geom_histogram() +
  facet_wrap(~neighbourhood) +
  labs(
    x = "Average rating score of property",
