...
- Continue practicing data tidying and visualisation
- Calculate summary statistics with `summarise()`
- Arrange output of dplyr chains with `arrange()`

### Getting help

If you have any questions about the assignment, post them on [Piazza](https://piazza.com).
...
```{r medians-less-than-avg, exercise=TRUE}
college_recent_grads %>%
  filter(
    major_type == "stem",
    median < 36000
...
```{r median-less-than-avg, exercise=TRUE}
college_recent_grads %>%
  filter(
    major_type == "stem",
    median < 36000
...
```{r majors-women-labels, exercise=TRUE}
ggplot(___ = college_recent_grads, 
       mapping = aes(x = ___, y = ___, color = ___)) +
  ___() +
  ___(
