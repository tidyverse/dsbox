...
grade_this({
  ...
  if(identical(floor(as.numeric(.result$lower[1])), 378) & identical(floor(as.numeric(.result$upper[1])), 460)) {
    fail("It looks like you've calculated a 98% confidence interval. Remember that to pick out hte central 99% of the distribution, we need to discard the most extreme 0.5% at each side.")
  }
  fail("Not quite. Take a look at the hints if you need some help.")
})
```
