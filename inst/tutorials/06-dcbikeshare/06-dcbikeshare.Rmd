...
description: >
  Learn modelling, interpretation of model performance and backward selection
...
#create mutated bike dataset for all exercises after wrangling
...
dcbikeshare <- mutatedbike
m_cnt_tmp <- lm(cnt ~ temperature_raw, data = dcbikeshare)
```

```{r rentals-vs-atemp-r2-setup}
dcbikeshare <- mutatedbike
m_cnt_atmp <- lm(cnt ~ feeling_temperature_raw, data = dcbikeshare)
```

```{r adj-r-squared-setup}
dcbikeshare <- mutatedbike
m_full <- lm(cnt ~ season + yr + holiday + workingday + weathersit + 
                   temperature_raw + feeling_temperature_raw + humidity_raw + windspeed_raw + feeling_temperature_raw * holiday, 
                   data = dcbikeshare)
```
...
In this tutorial, you will learn and expand the following skills:

  - Data wrangling
  - Modelling and interpretation of model performance
  - Backward selection
...
Consequently, the `weathersit` variable only has three factors.
...
### Bike rentals versus temperature
...
m_cnt_tmp <- lm(cnt ~ ___, data = dcbikeshare)
m_cnt_tmp %>%
  tidy()
```
...
```{r rentals-vs-temp-check}
	 grade_result(
   pass_if(~ identical(floor(.result$estimate[2]), 161), "You have written the model correctly"),
   fail_if(~ identical(floor(.result$estimate[1]), 9), "Did you maybe try to predict the weather from the number of bike rentals?"),
   fail_if(~ identical(floor(.result$estimate[1]),11), "Did you maybe try to predict the weather from the number of bike rentals?"),
   fail_if(~ identical(floor(.result$estimate[1]), 0), "Did you maybe try to predict the weather from the number of bike rentals?"),
...
```{r rentals-vs-atemp-check}
	 grade_result(
   pass_if(~ identical(floor(.result$estimate[2]), 150), "You have written the model correctly"),
   fail_if(~ identical(floor(.result$estimate[1]), 11), "Did you maybe try to predict the weather from the number of bike rentals?"),
   fail_if(~ identical(floor(.result$estimate[1]), 0), "Did you maybe try to predict the weather from the number of bike rentals?"),
   fail_if(~ identical(floor(.result$estimate[1]), 1214), "Did you use normalized temperature instead of raw temperature?"),
   fail_if(~ TRUE, "Not quite. Look at the hints for help!")
 )
``` 
...
```{r m-full-hint-1}
m_full <- lm(cnt ~ ___ + feeling_temperature_raw * holiday, 
                   data = ___)
tidy(m_full)
```
...
```{r m-full-check}
	 grade_result(
   pass_if(~ identical(floor(.result$estimate[1]), 2715), "You have written the model correctly"),
   fail_if(~ identical(floor(.result$estimate[1]), 2688), "Did you forget to model the interaction effect between feeling temperature and holiday?"),
   fail_if(~ TRUE, "Not quite. Look at the hints for help!")
 )
``` 
...
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 4)), 0.7491), "You have written the model correctly"),
   fail_if(~ TRUE, "Not quite. Look at the hint for help and remember to just remove the season variable from the full model!")
 )
``` 
...
```{r remove-year-check}
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 5)), 0.5536), "You have written the model correctly"),
   fail_if(~ TRUE, "Not quite. Remember to just remove the year variable from the full model!")
 )
``` 
...
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 4)), 0.8173), "You have written the model correctly"),
   fail_if(~ identical(as.numeric(round(.result, digits = 3)), 0.820), "Did you forget to remove the interaction effect?"),
   fail_if(~ TRUE, "Not quite. Did you use the full model as your starting point?")
 )
```
...
rm_workingday<- lm(___)
glance(___)$___
```

```{r remove-workingday-check}
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 4)), 0.8196), "You have written the model correctly"),
   fail_if(~ TRUE, "Not quite. Did you use the full model as your starting point?")
 )
```
...
rm_weathersit<- lm(___)
glance(___)$___
```

```{r remove-weathersit-check}
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 5)), 0.79847), "You have written the model correctly"),
   fail_if(~ TRUE, "Not quite. Did you use the full model as your starting point?")
 )
```
...
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 5)), 0.81813), "You have written the model correctly"),
   fail_if(~ TRUE, "Not quite. Did you use the full model as your starting point?")
 )
```
...
rm_feeltemp_raw <- lm(___)
glance(___)$___
```

```{r remove-feeltempraw-check}
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 5)), 0.81988), "You have written the model correctly"),
   fail_if(~ identical(as.numeric(round(.result, digits = 3)), 0.820), "Did you forget to remove the interaction effect?"),
   fail_if(~ TRUE, "Not quite. Did you use the full model as your starting point?")
 )
```
...
rm_humidity_raw <- lm(___)
glance(___)$___
```

```{r remove-humidityraw-check}
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 4)), 0.8151), "You have written the model correctly"),
   fail_if(~ TRUE, "Not quite. Did you use the full model as your starting point?")
 )
```
...
rm_windspeed_raw <- lm(___)
glance(___)$___
```

```{r remove-windraw-check}
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 4)), 0.8106), "You have written the model correctly"),
   fail_if(~ TRUE, "Not quite. Did you use the full model as your starting point?")
 )
```
...
rm_interaction <- lm(___)
glance(___)$___
```

```{r remove-interaction-check}
grade_result(
   pass_if(~ identical(as.numeric(round(.result, digits = 4)), 0.8197), "You have written the model correctly"),
   fail_if(~ identical(as.numeric(round(.result, digits = 4)), 0.8174), "Did you also remove the two variables individually in addition to removing the interaction effect?"),
   fail_if(~ TRUE, "Not quite. Did you use the full model as your starting point?")
 )
```
...
Congratulations, you finished tutorial 8!
