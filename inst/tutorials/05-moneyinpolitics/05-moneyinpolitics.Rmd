...
description: >
  Learn about data collection with webscraping with data on foreign-connected
...
```{r read-pac-all-orig}
pac_all_raw <- dsbox::pac_all_raw
```

```{r read-pac-all-clean}
pac_all <- dsbox::pac_all
```
...
- Introduce you to data collection via web scraping.
- Continue to build on your data cleaning and visualisation skills.

### Getting help

As always, if you have any questions around the assignment, post them on [Piazza](https://piazza.com).

### Packages

In this assignment we will work with the following packages: `tidyverse` and `scales` (which you should be familiar with), and `robotstxt` and `rvest` for web scraping. We also use the `dsbox` package for the data.
...
The functions returns `TRUE`, the answer is "yes", you're technically allowed to scrape the webpage.
...
knitr::include_graphics("images/Selector Gadget.PNG")
```
...
```{r pac-all-separate, exercise=TRUE, exercise.setup = "read-pac-all-orig", message=FALSE}
...
grade_code("You've created a function for cleaning the monetary values. Good job!")
```
...
```{r parse-currency-data, exercise=TRUE, exercise.setup = "read-pac-all-orig", message=FALSE}
pac_all <- ___ %>%
  ___(
    total = ___,
    dems = ___,
...
grade_code("Now all the monetary valuse are stored as numeric data.")
```
...
```{r glimpse-cleaned-all, exercise=TRUE, exercise.setup = "read-pac-all-clean", message=FALSE}
glimpse(pac_all)
```
...
```{r uk-canada-plot, exercise=TRUE, exercise.setup = "read-pac-all-clean", message=FALSE}
pac_all %>%
  ___(country %in% ___) %>%
  ___(country, year) %>%
  ___(tot = ___) %>%
...
```{r final-plot-filter, exercise=TRUE, exercise.setup = "read-pac-all-clean", message=FALSE}
pac_all %>%
  filter(
    country == "UK",
    year < 2020
...
```{r final-plot-total-contributions, exercise=TRUE, exercise.setup = "read-pac-all-clean", message=FALSE}
pac_all %>%
  filter(
    country == "UK",
    year < 2020
...
```{r final-plot-pivot, exercise=TRUE, exercise.setup = "read-pac-all-clean", message=FALSE}
pac_all %>%
  filter(
    country == "UK",
    year < 2020
...
```{r final-plot-data-frame, exercise=TRUE, message=FALSE, exercise.setup = "read-pac-all-clean", message=FALSE}
